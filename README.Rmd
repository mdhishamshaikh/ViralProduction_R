...
The `viralprod` package simplifies the analysis of viral production data, making it suitable for researchers studying marine viruses. To utilize the package effectively, it's essential to complete prior steps, including sampling, filtration, virus reduction assay and flow cytometry processing. The package encompasses **three** fundamental steps, each contributing to a comprehensive analysis:
...
Within the package, each of the three primary steps has its dedicated function. Additionally, a comprehensive wrapper function exists, `vp_end_to_end`, which integrates all three essential steps of viral production analysis into a single execution. Executing this singular function yields calculated viral production results, analyzed data, and visualizations. Below, we will demonstrate the functionality of the wrapper function using example data available in the `inst/extdata` folder, use of the separate main functions is integrated in the `Introduction to viralprod` vignette. 

Running the code, provided below, also serves as a convenient method to verify the correct installation of the package. Note that the assessment of input data frames conforms to the package's requirements is seamlessly integrated into the wrapper function through the `vp_class_count data` function. 
