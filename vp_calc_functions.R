...
## Added in every method a group_by statement and factor of Time_Range in arrange formula for consistent result dataframes
...
#4. Linear Regression - Averaged Replicate - Difference Curve (LM_AR_Diff) LM4 (DIFF = Subtraction) # ONLY difference here is the filtering on Sample_Type => is possible to make LM3 and LM4 in one function but I think it will make things more complicated so a general function for each method is still the best way to go, i guess
...
vp_vpcl_ar_se<- function(data){
  ...
  VPCL_AR_SE<- vipcal_avg_se(data_avg) %>%
    calc_diff_lm_AR() %>% # Wrong function was used here
    group_by(Location, Expt_No, Depth, Time_Range, Population, Sample_Type) %>%
    arrange('Location',
            'Expt_No',
            'Depth',
