...
test_that("Right populations are analyzed", {
  ...
  vp_check_populations(data_NJ2020_all) %>% expect_output_check_populations()
  length(populations_to_analyze) %>% expect_equal(length(which(startsWith(names(data_NJ2020_all),'c_'))))
  ...
  vp_check_populations(data_NJ2020_less) %>% expect_output_check_populations()
  length(populations_to_analyze) %>% expect_equal(length(which(startsWith(names(data_NJ2020_less),'c_'))))
  ...
  vp_check_populations(data_NJ2020_more) %>% expect_output_check_populations()
  length(populations_to_analyze) %>% expect_equal(length(which(startsWith(names(data_NJ2020_more),'c_'))))
